...
def gen_easy_password():
    """Generate easy password: 4-6 chars from restricted charset"""
    length = random.randint(4, 6)
    return "".join(random.choice(EASY_CHARSET) for _ in range(length))


def gen_medium_password():
    """Generate medium password: 7-8 chars from full alphanumeric"""
...
for name, gen_func in [
    ("easy", gen_easy_password),
    ("medium", gen_medium_password),
    ("hard", gen_hard_password),
]:
